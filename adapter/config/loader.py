...
def load_config(yaml_path: Path | None = None) -> AppConfig:
    ...
    kafka = KafkaConfig(
        ...
        publish_timeout_seconds=int(
            os.environ.get('INGEST_BRIDGE_KAFKA_PUBLISH_TIMEOUT_SECONDS')
            or str(kafka_raw.get('publish_timeout_seconds', 10))
        ),
    )
    ...
    return AppConfig(
        ...
        source_token=os.environ.get('INGEST_BRIDGE_SOURCE_TOKEN')
        or str(raw.get('source_token', '')),
        kafka=kafka,
    )
